...
<template>
  <aside 
    :class="[
      ...
      'w-60 flex-shrink-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 hidden md:flex flex-col sticky top-0 h-screen transition-all duration-300 ease-in-out z-40'
    ]"
  >
    ...
    <nav class="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
      ...
      <button v-for="cat in categories" :key="cat.slug" @click="selectedCategory = cat.slug" :class="[selectedCategory === cat.slug ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400']" class="w-full text-left px-4 py-2 rounded-xl text-sm font-medium capitalize hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
        {{ cat.name }}
      </button>
      ...
      <button @click="sortOrder = 'default'" :class="[sortOrder === 'default' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400']" class="w-full text-left px-4 py-2 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
        Default
      </button>
    ...
    <div class="p-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-800 transition-colors duration-300">
      ...
      <div class="max-h-64 overflow-y-auto space-y-3 mb-4 custom-scrollbar">
        <div v-for="item in cart" :key="item.id" class="flex items-center gap-3 bg-white dark:bg-slate-800 p-2 rounded-lg border border-slate-200 dark:border-slate-700 transition-colors">
          ...
          <button @click="removeFromCart(item.id)" class="text-slate-300 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition-colors">
      ...
      <button :disabled="cart.length === 0" class="w-full mt-3 py-2 bg-indigo-600 dark:bg-indigo-500 text-white text-sm font-bold rounded-xl disabled:opacity-50 hover:bg-indigo-700 dark:hover:bg-indigo-400 transition-colors">
        Checkout
      </button>
    </div>
